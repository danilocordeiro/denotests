...
const getStudent = (
  { params, response }: { params: { roll_no: string }; response: any },
) => {
...
const updateStudent = async (
  ...
  if (student) {
    ...
    students = [
      ...students.filter((student) => student.roll_no !== params.roll_no),
      student,
    ];
    response.status = 200;
...
const searchStudentByRoll = (roll_no: string): (IStudent | undefined) =>
  students.filter((student) => student.roll_no === roll_no)[0];
